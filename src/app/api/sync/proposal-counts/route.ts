 ...
 *
 * ⚠️ projects 커서는 건드리지 않는다. 완료 감지용 `proposal_counts` source의 last_run_at만
 *    기록하며 cursor_value는 항상 NULL이다.
 */
interface RawRow {
  id: number | string;
...
export async function POST(req: Request): Promise<Response> {
  ...
  // 지원수 완료 시각은 별도 source 로 기록한다. cursor_value 는 쓰지 않으므로 projects의
  // 증분 커서에는 영향이 없고, 홈 버튼은 이 시각이 전진한 뒤에만 화면을 새로고침한다.
  const result = await query<{ updated: number | string }>(
    `WITH updated AS (
       UPDATE projects p
       ...
       VALUES ('proposal_counts', NULL, now())
       ON CONFLICT (source) DO UPDATE SET last_run_at = now()
       RETURNING 1
     )
     SELECT count(updated.id)::int AS updated
       FROM marked
