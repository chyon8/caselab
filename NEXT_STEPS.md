...
| 매니저 코멘트 | **~129,000** | [`n8n/managenote_incremental.sql`](./n8n/managenote_incremental.sql) | `/api/sync/timeline` |
...
| 매니저 코멘트 | `managenote_incremental.sql` | `/api/sync/timeline` | `managenote` |
...
### 3-3. 매니저 코멘트 (~129,000건 / 645배치, 10~20분)

**확인:** `SELECT count(*) FROM timeline_events WHERE source='managenote';`
...
- 상세 화면 타임라인에 매니저 코멘트(일반/공지)와 개발사 Q&A가 함께 뜨는가?

### 결정 필요

**🔴 비공개 Q&A 노출 정책.** 개발사 댓글 21,324건 중 **18,780건(88%)이 비공개**다. 이걸 안 가져오면
"개발사가 뭘 묻는가"의 본체를 버리는 거라 **가져오기로 했지만**, 화면에 그대로 띄우면 원래
클라이언트만 보던 글이 보이게 된다. `meta.is_private`로 구분해 넘기고 있으니 화면 정책을 정해야 한다.

---

## 알려진 한계

**🟡 `proposal_count`는 낡는다.** 지원자가 새로 들어와도 본진 `project_project.date_modified`가
안 바뀌면 증분 동기화가 그 행을 다시 안 가져온다. 정확한 모집 퍼널이 필요하면 `proposal_proposal`을
별도 동기화해서 CaseLab에서 직접 세야 한다.
...
2. **매니저 코멘트 요약** — 데이터는 들어온다. 요약(AI)은 프롬프트 검토 대기.
...
| 3 | 비공개 Q&A 화면 노출 정책 | 상세 화면 Q&A 섹션 |
