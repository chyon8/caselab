...
> ⚠️ **2026-07-16 결정: 통화 트랙 후순위로 확정, 아래 "내일 할 일" 무효화됨.**
> 이유: (1) project_id 매핑이 by-phone 방식의 구조적 한계 — 파트너·매니저 번호가 여러 프로젝트에 걸치고,
> 한 통화 안에서도 여러 프로젝트가 섞여 언급됨. confidence=high여도 오염 확인됨(예: project 153821에
> 무관한 타 프로젝트 통화 다수 혼입). (2) 미팅(`meeting_meeting`)은 소스에 project_id가 직접 있어
> 매핑이 공짜로 정확한 반면, 통화는 추측이라 구조적으로 열등함. (3) 지급·정산 이슈가 내용의 다수인데
> 이건 검수 매니저가 개입할 수 있는 리스크가 아님(목표 범위 밖). (4) 껍데기 행(빈 summary/transcript)이
> 35.7%(226/633)로 신호 밀도도 낮음.
> **조치:** UI 제거(`ProjectDetail.tsx`의 `CallCard`/통화 섹션 렌더 삭제, 2026-07-16) — 데이터는
> `calls` 테이블에 보존만 함, 자동 리스크 태깅 등 다운스트림에 사용 금지. 전체 백필도 보류.
> 재개하려면: client 통화 × `meeting_meeting`에서 그 번호가 프로젝트 하나에만 걸리는 경우로 범위를
> 좁혀 매핑을 신뢰 가능하게 만드는 방안부터 (지금 당장 할 일 아님).
>
...
> ⚠️ **2026-07-16 정정: 아래 "완전히 비어있다" 전제 stale.** 실측 `timeline_events` = **17,025건**
> (qna 16,786 + meeting 200 + change 7 + status 32). 미팅 파이프라인은 최소 1회 실행되어 200건 이미
> 유입됨. change/status 건수가 적은 이유는 아래 설명대로 맞다 — 백필 후 실제로 값이 바뀐(지금도
> 살아 움직이는) 소수 프로젝트에서만 파생 생성되기 때문. 메커니즘 자체는 정상 확인됨(예: project
> 156662 계약금액 null→3천만원 이벤트). "타임라인 엔진에 불 켜기"는 이미 부분적으로 켜져 있다 —
> 남은 건 죽은 프로젝트 대신 살아있는 프로젝트의 이벤트를 늘리는 것(=미팅/노트 유입 확대).
