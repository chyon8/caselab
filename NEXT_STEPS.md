...
4. **회고적 데이터다.** `work_scope`는 계약 **체결 이후**에만 생긴다. 검수 매니저의 실시간 판단
   (검수→모집 구간)엔 안 쓰인다.
5. **§④가 원래 원하려던 "과업 팽창률"은 텍스트가 아니라 숫자(`total_price`)로 나온다.** AI도 파싱도
   필요 없다 — 관통 원칙(§⑥ 하단) 그대로: 숫자는 SQL이 공짜다.
...
## "미팅 녹취 vs 통화" — 헷갈릴 만하다, 정리해둔다

**결론: 둘은 별개 데이터가 아니다. 위시켓에 "미팅 녹취"라는 독립 소스는 없다.**

- `meeting_meeting` 테이블(`DATA_SCHEMA.md` §8-1)에는 **녹취·transcript 컬럼이 아예 없다.** 미팅의 메타데이터(일시·방식·장소·연락처)만 있다.
- 말(음성/텍스트)이 남는 **유일한 원천은 통화 API**(§8-3, `/api/calls/by-phone/`)뿐이다. 그래서 `DATA_SCHEMA.md` §8 제목도 "**개발사 사전 미팅 기록 및 녹취 API**" — 처음부터 미팅과 통화녹취를 같은 절로 묶어놨다.
- 실제 받아온 것: **위시켓 매니저가 클라이언트/파트너에게 건 전화의 STT.** (예: "차현지 매니저가 고객에게…", "김광현 매니저가…") 즉 매니저의 매칭·조율·CS 통화다.
- ⚠️ **주의할 점 하나:** 이건 클라이언트↔파트너가 직접 나눈 기술 미팅의 녹음이 아니다. 대면(method 3)·화상(method 4) 미팅은 어디에도 녹음이 없다. 위시켓 미팅(method 0)은 매니저가 끼므로 매니저 통화가 사실상 그 내용이 되지만, "두 당사자의 순수 미팅 녹음"을 원했다면 그 데이터는 위시켓에 존재하지 않는다.

**→ 방향이 틀린 게 아니다. 통화 API가 "미팅 녹취"에 가장 근접한(그리고 유일한) 소스가 맞다.** 다만 전화번호로 조회하다 보니 그 사람의 다른 프로젝트 통화·일반 영업 통화도 딸려 오고, 그래서 API가 `project_id`를 **추측**해서 붙이고 `confidence`를 매긴다. 진짜 신경 쓸 건 통화↔프로젝트 매핑 정확도이지, "통화냐 미팅이냐"가 아니다.
