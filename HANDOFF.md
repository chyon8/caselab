...
> 새 세션 시작 시 이 파일 먼저. 최종 업데이트: **2026-09-23**
...
> **재착수 기준 — 2026-09-23:** 이전 넓은 수요 태그 분석은 사용자 목표와 맞지 않아 결과·보고서·분류 코드·대시보드를 제거했다. 새 세션은 반드시 [`analysis/target-demand/NEXT_SESSION.md`](./analysis/target-demand/NEXT_SESSION.md)를 먼저 읽고, 사용자 지시 전에는 분석·앱 구현을 시작하지 않는다.

- **사용자 목표:** 실제 공고에서 `업종 × 구체 업무/시스템·문제` 조합을 찾아, 모집과 계약을 각각 근거로 우선 검토 타깃을 판단한다. 물류·내부 시스템은 형식의 예시이며, 분석 범위를 제한하는 조건이 아니다. 마케팅 실행안은 제외한다.
- **Git에 남긴 것:** `analysis/target-demand/`의 비식별 원문 `corpus.json`, 성과 원장 `outcomes.json`, 원천 대조 `audit.json`·`data-manifest.json`, 재착수 기준 문서뿐이다. 실제 프로젝트 번호 매핑은 Git에 없다.
- **아래 이전 상세 기록은 폐기된 분석의 이력이며 사용 금지다.** 삭제된 보고서·태그·대시보드 링크를 복원하거나 재사용하지 않는다.
- [최종 보고서](./analysis/target-demand/REPORT_2026_01_08.md), [28종 전체 근거표](./analysis/target-demand/EVIDENCE_TABLES.md), [재현 검증](./analysis/target-demand/verification.json). 본문3,737건 처리 완료: GPT-5.5 high API3,199 + 빈 양식 규칙536 + 명시된 Codex 원문 판독2건. 판독가능3,070건(양식/수요불명667), 범주밖438건도 비교군에 포함.
- **결론:** 기존 서비스 오류·성능·기능 보완을 우선 검토(393건, 모집265/393=67.4%, 같은 월×출처 대비 +12.4%p, 8개월 모두 양수). 계약은 제출90일72/289=24.9%, 모집90일72/191=37.7%. 계약 우위는 반복 고객에서 더 뚜렷해 신규 유입에 일반화 금지. 화면/프런트 한정과 소개홈페이지는 최근 월 약화로 조건부. 결제연동·고객이력은 분류 경계 문제로 추가 확인 후보.
- 스크립트 `target-demand*.mjs`: 추출/분류, 분모·민감도, 검증/수정 기록, 근거표, 후보 선별, 재현 확인. 테스트11개 통과, 본문 이메일/전화번호/키 패턴0건, 표본 중복0건, 집계/근거표 재생성 해시 일치. 앱에는 정적 분석 대시보드 [`/target-demand`](./src/app/(app)/target-demand/page.tsx)를 추가했다. DB·운영 API·스키마는 변경하지 않으며, 집에서도 같은 스냅샷을 볼 수 있다.
- **발견/분류 검증:** 사용자 “다 하고 나한테 보고해” 답변 후 gpt-4o-mini 실행. 발견200건 완료(199 API +1 Codex 직접 판독), 수요28종 `taxonomy.json` 동결. 별도80건을 Codex가 성과/API라벨 없이 원문 판독해 `validation-reference.json` 작성(사람 정답 아님). 전수분류 중 과잉분류 발견으로 2,626건 시점에 중단·비공개 보관, 사용 금지. 엄격한 프롬프트 대안도80건 완전일치32/80·정밀도60.7%·재현율43.6%로 미달.
- **모델 승인·검증:** 사용자 “모델 올려”, “더 올려도돼. 좋은 결과만 나오면 돼”로 상향 승인. 기존 OpenAI API `gpt-5.5` high/store:false/성과 제외. 최종 내부 검증68/80·정밀도88.0%·재현율93.6%. 동일80건으로 경계 프롬프트를 교정해 독립 정확도 추정이 아니다. 사람 검증으로 표현 금지. 기준표/통과선 고정, 첫 실행 보존. 후보60개 태그(54개 공고) 원문 대조, 총7건 수정 기록 및 원래 라벨 보존.
- **남은 필수 분석 없음:** 사용자는 보고서만 검토하면 된다. 가명 입력·라벨·결과·스크립트·문서는 Git 공유 대상이지만 **AI는 Git 명령을 실행하지 않았다**. 검토 컨펌 후 필요한 파일의 커밋/푸시 명령을 제공한다. 집에서 분석하려면 해당 변경을 먼저 공유해야 하며, 이미 푸시됐다고 가정하지 않는다. 전체 API 재호출 불필요.
- **주의:** 모집 본문 확보율100%, 모집일 없는 집단75.6%. 본문 분석을 전체 제출로 일반화하지 않는다. 제출 전 계약일7건은 이번 코드에서 미상 처리(기존 보고서는 실패 포함); 제출90일 계약406/3,234.
- 예전 준비도 TSV의 두 사람 검증은 이번 분석의 선행조건이 아니다. 새 수요 분류와 별개다.

## 이전 제출→모집 패턴 분석 — 구조화 분석 완료 (2026-09-22)

- [실행 계획](./analysis/submission-conversion/README.md), [변수 정의서](./analysis/submission-conversion/CODEBOOK.md), [최종 구조화 결과](./analysis/submission-conversion/RESULTS_2026_01_08.md), 재현 스크립트 작성. 앱/운영 화면은 변경하지 않았다.
- 사용자 결정: **2026년 1~8월 제출 외주**가 본 분석. 기업 규모는 자료 제공이 어려워 **이번 분석에서 제외**. 기존 본진 스키마의 기획·인력·일정·자금 후보는 우선 수집한다. **1시간 연락 효과·영업시간 분석 제외**.
- **추가 우선순위:** DB 접근은 회사에서만 가능. 연휴/주말 전 **핵심 실제 데이터 추출·비식별화·승인된 저장 경로·회사망 밖 열람 검증**을 끝내고, 분류/분석은 집에서 수행. 66개 항목 전부 확보를 강제하지 않으며 쉽게 얻는 자료는 함께 수집, 고비용 보강은 후순위. 세부 순서는 [실행 계획 §0](./analysis/submission-conversion/README.md#0-실행-우선순위--회사에서는-수집-집에서는-분석).
- Neon read-only 실측: 이 기간 CaseLab 1,931건 모두 모집일 있음. 초기 예산 양수 484건(25.1%), 초기 기간 양수 607건(31.4%), planning_status 값 0건. 전체 제출 분모/미전환 비교군은 없으며 최신 공고를 제출 원문으로 쓰면 안 된다.
- 준비도 단일 점수 대신 정보 구체성·추진 여건·프로젝트 내용을 분리한다. 근거 없는 값은 미상, 유의미한 패턴/가설 0개도 허용. 반복 고객·누락·시점·판독 검증·신뢰구간의 적용 범위를 사전 정의했다.
- 본진 월별 점검 실측: 전체 제출 **4,319건**, 모집일 있음 **1,932건(44.7%)**. CaseLab 기존 모집 표본 1,931건과 1건 차이는 본 분석 원장 적재 후 대조한다.
- 분석 적재 4,319건 백필·월별 원천 대조 완료. 주 지표는 14일 내 모집 1,901건, 상태 충돌 18건을 반영한 관측 범위 44.0~44.4%. 7/30일·누적, 첨부·과거 이력·사업 형태·유입·월별·90일 계약 가드레일과 95% CI까지 `scripts/analyze-submission-conversion.mjs`로 재현한다.
- 사용자 제공 7~8월 원장은 도급 646건(모집175·거절428·직접취소43) 매칭. 원본은 Git에서 제거해 `.private/submission-conversion/`에 보관하며, 집계만 Git에 남긴다. 과거 Git 이력 제거는 사용자 실행 대기.
- 제출 전 스냅샷은 1,007건만 검증됐고 원문은 859건. 고정 seed로 교정40·검증80·나머지739를 분리했다. gpt-4o-mini 교정 중 폼 질문 오분류를 발견해 전처리/프롬프트를 수정하고 교정40·검증80 AI 분류를 완료했다. 두 사람의 독립 판독과 정확도 검증 통과 전에는 원문 라벨을 결과에 사용하지 않는다.
- 집/회사 연속 작업: 구조화 결과·보고서·스크립트와 개인정보 제거된 익명 원문 검증 자료는 Git 대상이다. 프로젝트 ID 매핑과 원본 7·8월 CSV는 `.private/`에만 둔다.
...
> **2026-09-16 사용자 지시:** 매니저 노트 동기화는 연결 완료. 아래 데이터 확장과 기존 백로그는
> 당분간 보류하고, 홈 "지금 동기화" 실행 시 모집 중 프로젝트의 지원자 수도 함께 갱신하는 작업을
> 우선한다. 저장소 구현은 `proposal_counts` 완료 시각 기록 + 화면 자동 새로고침까지 반영했으며,
> 라이브 n8n 메인 웹훅의 일반 프로젝트 적재 뒤에 `proposal_counts_refresh.sql` →
> `/api/sync/proposal-counts`를 연결한다. `/query`의 item 1개 안 `data` 배열을 API의 `rows`로 보낸다.
> 실제 노드 구성은 [`n8n/projects_pipeline.md`](./n8n/projects_pipeline.md).
...
**④ 검수 스코어링 개선 3종 — 기획 확정, 구현 미착수 (2026-08-12).** 순서는 **② 기능별 예산 → ① 질문의 답에 따른 금액 변동 → ③ 견적 안내 메일 초안**. 취향이 아니라 의존 관계다 — ①의 금액 델타도 ③의 메일 본문도 «기능 단위 견적»을 재료로 쓴다. 지금 `qty`는 기능 이름 없는 숫자 뭉치라 쪼갤 수가 없고, `questions.ts`와 `estimate.ts`는 서로를 모른다. 설계·결정·가드는 NEXT_STEPS "검수 스코어링 개선 3종".
...
## 🏁 사용자 결정으로 종료 (2026-09-16)

- **DB owner URL 노출 처리** — 외부 참조의 `neondb_owner` URL 교체·비밀번호 로테이션은 **사용자 결정으로 종료**. 실제 교체·로테이션 여부는 확인하지 않았으며, 추가 조치를 추적하지 않는다.
- **n8n 스케줄 확정** — 실제 n8n 캔버스의 주기 확인·변경은 **사용자 결정으로 종료**. 워크플로 문서의 15분 주기는 설계 참조이며, 운영 적용이 확인됐다는 뜻은 아니다.
- **배치 트렁케이션 감지 하드닝** — `expected` 필드로 전송 누락을 감지하는 변경은 **미구현 상태로 사용자 결정에 따라 종료**. Q&A·매니저 노트 동기화에 이 보호 장치는 없다.

---

## ⚠️ 블로커 / 대기 / 잊지 말 것

- ✅ ~~OpenAI 크레딧 소진 (2026-08-12)~~ — **해소.** 개인 키가 소진돼 회사 키로 교체됨. 2026-08-12 실측 `200 OK`.
- ✅ ~~`scripts/*.mjs`의 키 읽기 방식이 위험하다~~ — **해소 (2026-08-12).** `env.match(/OPENAI_API_KEY=(.*)/)`는 파일에서 **처음 걸리는 줄**을 잡고 정규식이 `#`을 몰라, 주석 처리해 둔 옛 키로 800건을 돌린 사고가 실제로 났었다. [`scripts/env.mjs`](./scripts/env.mjs) `readEnv()`로 교체 — **줄머리에서만** 찾고(주석 줄은 애초에 안 걸린다), 없거나·여러 줄이거나·값이 비면 던진다. `DATABASE_URL`도 같은 결함이라 함께 고쳤다. 7개 스크립트 전부 적용.
- **DB에 클러스터 잔재가 남아 있다** — `clusters` 테이블 + `projects.cluster_id`. 읽는 코드는 전부 지웠으니 무해하다. [`migrations/020_drop_clusters.sql`](./migrations/020_drop_clusters.sql) 준비됨 — **Neon 콘솔 적용 대기.**
