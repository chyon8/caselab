...
- **n8n 스케줄 확정** — 워크플로가 소스별로 구성돼 있으나 **스케줄 주기가 미확정**이다. 안 걸려 있으면 동기화 버튼을 눌러야만 데이터가 들어오고, cron은 할 일이 없다. 사전 미팅 녹취(`meeting_transcripts`)는 매 실행이 최근 60일을 전량 재스캔·멱등 upsert라 크론을 걸어도 무해하다. **managenote 동기화 연결은 2026-09-16 사용자 확인 완료.**
