...
## 제출→모집 패턴 분석 — 실행 계획 작성, 원천 확인 대기 (2026-09-22)

- [실행 계획](./analysis/submission-conversion/README.md), [필요 데이터 66개 정보 묶음](./analysis/submission-conversion/DATA_INVENTORY.md), [변수 정의서](./analysis/submission-conversion/CODEBOOK.md), 읽기 전용 SQL 5개와 CSV 양식 2개 작성. **분석 결과가 아니라 분석·검증 절차**다. 앱/운영 동기화는 변경하지 않았다.
...
- 분석 적재 4,319건 백필 완료·검증: 모집일 있음 1,932건(44.7%)과 KST 월별 원천 집계가 일치했다. [1차 결과](./analysis/submission-conversion/RESULTS_2026_01_08.md) 작성. 사용자 제공 7~8월 검수 원장도 도급 외주 646건(모집175·거절428·취소43) 매칭해 사유별 보조 분석까지 반영했다. 원본은 `analysis/submission-conversion/input/`에 보관한다. `project_projectinitialvalue`의 3,294건은 제출 뒤 생성/수정돼 T0로 쓸 수 없었다. `detail_projectdetail`에는 실제로 작성 시각 컬럼이 없어 T0 판별이 불가하며 주 분석에서 제외했다. 운영 `projects`에는 섞지 않는다. 고객 ID/해시는 전송하지 않고 자유서술 연락처는 수신 시 스크럽한다.
- 검증: 분석 수신 라우트 포함 프로덕션 빌드 통과, 적재 컬럼58개와 값58개 일치, migration 컬럼 존재 확인. 본진 백필 SQL의 실제 컬럼·성능·값 검증은 n8n 첫 배치 전까지 미완료.
