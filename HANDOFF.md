...
- 분석 적재 4,319건 백필 완료·검증: 모집일 있음 1,932건(44.7%)과 KST 월별 원천 집계가 일치했다. 다만 `project_projectinitialvalue`의 3,294건은 제출 뒤 생성/수정돼 T0로 쓸 수 없었다. `detail_projectdetail.date_created`를 추가 수집하는 v2 보강은 구현 완료·배포/`022` 적용 대기다. 운영 `projects`에는 섞지 않는다. 고객 ID/해시는 전송하지 않고 자유서술 연락처는 수신 시 스크럽한다.
