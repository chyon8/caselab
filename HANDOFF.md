...
- 분석 적재 구현 완료(미배포): `submission_analysis_projects` 전용 테이블, `/api/sync/submission-analysis`, n8n 200건 페이지 백필 SQL/절차. 운영 `projects`에는 섞지 않는다. 고객 ID/해시는 전송하지 않고 자유서술 연락처는 수신 시 스크럽한다. 다음은 사용자 확인 후 배포·`021` 적용, n8n 22회 백필. 본진 실데이터 백필은 아직 미실행.
